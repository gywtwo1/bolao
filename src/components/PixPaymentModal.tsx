...
export const PixPaymentModal: React.FC<PixPaymentModalProps> = ({ isOpen, onClose, betId }) => {
  const { activeRound, currentUser, submitPixReceipt, createNewBetForRound, userRoundBets } = useBolao();
  ...
  const handleCreateAnotherAndClose = () => {
    setSubmittedSuccess(false);
    onClose();
    createNewBetForRound(activeRound.id);
  };
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                <span>Pagamento PIX da Rodada</span>
                {currentBet && (
                  <span className="text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full font-bold">
                    {currentBet.betLabel || 'Palpite #1'}
                  </span>
                )}
              </h3>
          ...
          <div className="p-6 text-center space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-300 max-w-sm mx-auto leading-relaxed">
                Seu bilhete <strong>{currentBet?.betLabel || 'Palpite #1'}</strong> está em análise pelo Administrador e logo será validado no ranking.
              </p>
            </div>
            ...
            <div className="bg-slate-950/70 border border-slate-800 rounded-2xl p-4 text-xs text-slate-400 space-y-2">
              <p className="font-bold text-amber-300 flex items-center justify-center gap-1.5">
                ...
                Quer aumentar suas chances de vencer a rodada?
              </p>
              <p className="text-[11px] text-slate-300">
                Você pode fazer quantos palpites quiser nesta rodada (R$ 10,00 cada). Cada bilhete concorre individualmente e o seu melhor palpite entrará no ranking oficial!
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
              <button
                onClick={handleClose}
                className="w-full sm:w-auto px-5 py-2.5 rounded-xl border border-slate-700 text-xs font-bold text-slate-300 hover:bg-slate-800 transition-colors"
              >
                Concluir e Ver Meus Palpites
              </button>

              <button
                onClick={handleCreateAnotherAndClose}
                className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-slate-950 font-extrabold text-xs py-2.5 px-5 rounded-xl shadow-lg shadow-emerald-950/60 transition-all active:scale-95"
              >
