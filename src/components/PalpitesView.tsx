...
export const PalpitesView: React.FC = () => {
  ...
  const isLocked = isBettingClosed || activeBet?.isLocked || (activeBet && (activeBet.status === 'confirmed' || activeBet.status === 'receipt_submitted'));
  const betStatus = activeBet?.status || 'locked_pending_payment';
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[9px] uppercase px-1.5 py-0.5 rounded font-black ${
                    ...
                    {b.status === 'confirmed' ? 'Confirmado' : b.status === 'receipt_submitted' ? 'Em Análise' : 'Aguardando PIX'}
                  </span>
                  {userRoundBets.length > 1 && b.status !== 'confirmed' && b.status !== 'receipt_submitted' && (
                    <span
                      role="button"
                      tabIndex={0}
                      title="Excluir este bilhete não pago"
                      onClick={(e) => {
                        e.stopPropagation();
                        if (confirm(`Deseja excluir o ${b.betLabel || 'palpite'} pendente?`)) {
                          deleteDraftBet(b.id);
                        }
                      }}
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-300 mt-1 max-w-xl leading-relaxed">
              Placar Exato = <strong className="text-emerald-400">3 Pontos</strong> • Acerto de Vencedor/Empate = <strong className="text-emerald-400">1 Ponto</strong>. Preencha todos os 10 jogos para travar e pagar a taxa de {formatCurrency(activeRound.price || 10.00)} via PIX. <strong className="text-amber-400">Palpites sem confirmação do PIX não contam como prêmio!</strong>
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 bg-slate-950/80 border border-slate-800 p-3 rounded-2xl shrink-0">
            <div>
              ...
              <p className="text-base sm:text-xl font-black text-amber-300 font-mono">
                {formatCurrency(activeRound.totalPot || 0)}
              </p>
              <span className="text-[10px] text-slate-400 block font-semibold mt-0.5">
                {bets.filter(b => b.roundId === activeRound.id && b.status === 'confirmed').length}{' '}
                {bets.filter(b => b.roundId === activeRound.id && b.status === 'confirmed').length === 1 ? 'PIX confirmado' : 'PIX confirmados'}
              </span>
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="text-xs">
                ...
                <p className="text-slate-300 mt-1">
                  ...
                  {betStatus === 'locked_pending_payment' && (
                    isBettingClosed
                      ? 'O horário limite da rodada encerrou. Conforme o regulamento, não é mais permitido enviar comprovantes após o término do prazo.'
                      : 'Você travou seus 10 palpites! Pague R$ 10,00 via PIX e envie o comprovante para validar sua vaga.'
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {(betStatus === 'locked_pending_payment' || betStatus === 'rejected') && (
                isBettingClosed ? (
                  <div className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-950/60 border border-rose-500/40 text-rose-300 text-xs font-bold shrink-0">
                    <Lock className="w-3.5 h-3.5 text-rose-400" />
                    <span>Envio Encerrado (Fora do Prazo)</span>
                  </div>
                ) : (
                  <button
